...
//    }
    ...
    @IBAction func loginAction(_ sender: UIButton) {
        ...
        if loginSegmentedControl.selectedSegmentIndex == 0 {
            ...
            UserService.shared.storeAdminLogin(storeAcc: account, storePwd: password, storeID: storeID) { response in
                DispatchQueue.global(qos: .userInitiated).async {
                    URLCache.shared.removeAllCachedResponses()
                    DispatchQueue.main.async {
                        ...
                        guard response.code == returnCode else {
                            HUD.hideLoadingHUD(inView: self.view)
                            let errorMsg = response.responseMessage
                            Alert.showMessage(title: "", msg: errorMsg, vc: self, handler: nil)
                            return
                        }
